...
public struct CID:Equatable {
    ...
    public init(_ cid:[UInt8], base:BaseEncoding? = nil) throws {
        if let mh = try? Multihash(cid), mh.algorithm == .sha2_256, mh.length == 32 {
            ...
            try self.init(version: ver, codec: multicodec.codec, hash: hash, multibase: base ?? (ver == .v0 ? .base58btc : .base32))
        }
    }
...
public func ==(lhs:CID, rhs:CID) -> Bool {
    return lhs.rawBuffer == rhs.rawBuffer
}
...
public extension CID {
    ...
    mutating func toV0() throws {
        ...
        guard self.multihash.algorithm == .sha2_256, self.multihash.length == 32 else { throw CIDError.invalidV0Multihash }
